...
def get_quarterly_financials(ticker_symbol):
    """
    Fetches quarterly financials for trend analysis.
    """
    try:
        if not ticker_symbol.endswith('.BK'):
            full_ticker = f"{ticker_symbol}.BK"
        ...
        q_fin = stock.quarterly_financials.T.sort_index(ascending=True)
        q_cf = stock.quarterly_cashflow.T.sort_index(ascending=True)
        
        # Merge if possible, but primarily we want trends
        # Simple data structure for plotting
        data = pd.DataFrame()
        
        if not q_fin.empty:
            # Net Income
            if 'Net Income' in q_fin.columns:
                data['Net Profit'] = q_fin['Net Income']
            elif 'Net Income Common Stockholders' in q_fin.columns:
                data['Net Profit'] = q_fin['Net Income Common Stockholders']
                
            # Revenue
            if 'Total Revenue' in q_fin.columns:
                data['Revenue'] = q_fin['Total Revenue']
                
        if not q_cf.empty:
             if 'Operating Cash Flow' in q_cf.columns:
                 # Align indexes
                 # Reindex data to include CFO
                 data = data.join(q_cf[['Operating Cash Flow']], how='outer')
        
        # Calculate YoY Growth
        # Need at least 5 quarters to calculate YoY for the last 4
        if len(data) >= 5:
            data['Revenue Growth YoY'] = data['Revenue'].pct_change(periods=4) * 100
            data['Profit Growth YoY'] = data['Net Profit'].pct_change(periods=4) * 100
            
        return data
        
    except Exception as e:
        print(f"Error fetching quarterly for {ticker_symbol}: {e}")
        return pd.DataFrame()

def calculate_dupont(ticker_symbol):
    """
    ...
    try:
        ...
        fin = stock.financials.T.sort_index(ascending=True)
        bs = stock.balance_sheet.T.sort_index(ascending=True)
        
        if fin.empty or bs.empty:
            return None
            
        # Common years
        years = fin.index.intersection(bs.index)
        fin = fin.loc[years]
        bs = bs.loc[years]
        ...
        for dt in years:
            ...
            ni = fin.loc[dt, 'Net Income'] if 'Net Income' in fin.columns else (fin.loc[dt, 'Net Income Common Stockholders'] if 'Net Income Common Stockholders' in fin.columns else 0)
            rev = fin.loc[dt, 'Total Revenue'] if 'Total Revenue' in fin.columns else 0
            net_margin = ni / rev if rev > 0 else 0
            
            # 2. Asset Turnover = Revenue / Total Assets
            assets = bs.loc[dt, 'Total Assets'] if 'Total Assets' in bs.columns else 0
            asset_turnover = rev / assets if assets > 0 else 0
            
            # 3. Equity Multiplier = Total Assets / Equity
            equity = bs.loc[dt, 'Stockholders Equity'] if 'Stockholders Equity' in bs.columns else (bs.loc[dt, 'Total Equity Gross Minority Interest'] if 'Total Equity Gross Minority Interest' in bs.columns else 0)
        ...
        print(f"Dupont Error {ticker_symbol}: {e}")
        return None

# --- PHASE 2 ADVANCED FEATURES ---

SECTOR_PEERS = {
    'ENERG': ['PTT', 'PTTEP', 'TOP', 'BCP', 'IVL'],
    'BANK': ['KBANK', 'SCB', 'BBL', 'KTB', 'TTB'],
    'COMM': ['CPALL', 'CPAXT', 'CRC', 'BJC', 'COM7'],
    'ICT': ['ADVANC', 'TRUE', 'INTUCH'],
    'TRANS': ['AOT', 'BEM', 'BTS'],
    'PROP': ['LH', 'SIRI', 'SPALI', 'AP', 'ORI'],
    'FOOD': ['CPF', 'TU', 'MINT', 'CBG', 'OSP'],
    'HELTH': ['BDMS', 'BH', 'BCH'],
}

def get_competitors(ticker_symbol):
    """
    Finds competitors based on manual sector mapping or fallback to nothing.
    Returns list of ticker strings (e.g. ['KBANK', 'BBL'])
    """
    try:
        # Simplify ticker
        clean_ticker = ticker_symbol.replace('.BK', '').upper()
        
        # 1. Check known sectors
        found_peers = []
        for sector, peers in SECTOR_PEERS.items():
            if clean_ticker in peers:
                # Return all peers excluding self
                found_peers = [p for p in peers if p != clean_ticker]
                break
        
        # 2. Limit to 3 for UI sake
        return found_peers[:3]
        
    except Exception as e:
        print(f"Competitor Error: {e}")
        return []

def get_shareholders(ticker_symbol):
    """
    ...
    try:
        ...
        # Major Holders (returns DF)
        major = stock.major_holders
        
        # Institutional Holders (returns DF)
